    ...
    def add(self, loaded, thisSize):
        ...
        if self.data == None:
            ...
            self.data = dict(
               ...
               labels  = loaded['y'][:thisSize].astype('float32'),
            
               weights = loaded['weight'][:thisSize].astype('float32'),
          
               mvaid   = loaded['mvaid'][:thisSize].astype('float32'),
            )
            ...
            self.data['labels']  = np.concatenate((self.data['labels'],  loaded['y'][:thisSize].astype('float32')))
            self.data['weights'] = np.concatenate((self.data['weights'], loaded['weight'][:thisSize].astype('float32')))
            self.data['mvaid']   = np.concatenate((self.data['mvaid'],   loaded['mvaid'][:thisSize].astype('float32')))
    ...
    def add(self, loaded, thisSize):
        if self.data == None:
            ...
            for varname in self.varnames:
                ...
                self.data[varname] = loadedVar[:thisSize].astype('float32').reshape((-1,1))
        else:
            #----------
            # append
            #----------
            ...
            for varname in self.varnames:
                loadedVar = self.__getVar(loaded, varname)
                self.data[varname] = np.concatenate([ self.data[varname], loadedVar[:thisSize].astype('float32').reshape((-1,1)) ])
...
class SimpleVariableConcatenatorToMatrix:
    ...
    def add(self, loaded, thisSize):
        if self.data is None:
            ...
            for varindex, varname in enumerate(self.varnames):
                self.data[:, varindex] = loaded[varname][:thisSize]
            ...
            for varindex, varname in enumerate(self.varnames):
                newData[:,varindex] = loaded[varname][:thisSize]
    ...
    #----------------------------------------
    def makeAbsValue(self, varIndex):
        self.data[:,varIndex] = np.abs(self.data[:,varIndex])
...
class SparseConcatenator:
    ...
    def add(self, loaded, thisSize):
        ...
        if thisSize < loaded[self.groupVarName + "/" + self.firstIndexVar].shape[0]:
            ...
            objectsEndIndex = loaded[self.groupVarName + "/" + self.otherVars[0]].shape[0]
        ...
        if self.data == None:
            ...
            self.data[self.firstIndexVar] = loaded[self.groupVarName + "/" + self.firstIndexVar][:thisSize] - 1
            self.data[self.numItemsVar] = loaded[self.groupVarName + "/" + self.numItemsVar][:thisSize]

            # copy the sparsified data
            for varname in self.otherVars:
                self.data[varname]      = loaded[self.groupVarName + "/" + varname][:objectsEndIndex]
            ...
            numPhotonsBefore = self.data[self.firstIndexVar].shape[0]
            numObjectsBefore = self.data[self.otherVars[0]].shape[0]

            # append sparse objects (e.g. rechits, tracks)

            # copy the sparsified information
            for varname in self.otherVars:
                self.data[varname]      = np.concatenate([ self.data[varname],      loaded[self.groupVarName + "/" + varname][:objectsEndIndex] ])
      
            # copy the indices and lengths
            # we subtract one here from firstIndex
            self.data[self.firstIndexVar] = np.concatenate([ self.data[self.firstIndexVar], loaded[self.groupVarName + "/" + self.firstIndexVar][:thisSize] - 1 ])
            self.data[self.numItemsVar] = np.concatenate([ self.data[self.numItemsVar], loaded[self.groupVarName + "/" + self.numItemsVar][:thisSize] ])
